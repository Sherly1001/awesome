...
local tag_names = { 'S', 'h', 'e', 'r', '-', 'N', 'o', 'o', 'b' }
...
local numpads = {87, 88, 89, 83, 84, 85, 79, 80, 81, 90}
...
for i = 1, 9 do
    globalkeys = gears.table.join(globalkeys,
        ...
        -- View tag only with numpads.
        awful.key({ modkey }, '#' .. numpads[i], function()
                        local screen = awful.screen.focused()
                        local tag = screen.tags[i]
                        if tag then
                           tag:view_only()
        ...
        -- Toggle tag display with numpads.
        awful.key({ modkey, 'Control' }, '#' .. numpads[i], function()
                      local screen = awful.screen.focused()
                      local tag = screen.tags[i]
                      if tag then
                         awful.tag.viewtoggle(tag)
        ...
        -- Move client to tag with numpads.
        awful.key({ modkey, 'Shift' }, '#' .. numpads[i], function()
                      if client.focus then
                          local tag = client.focus.screen.tags[i]
                          if tag then
                              client.focus:move_to_tag(tag)
        ...
        -- Toggle tag on focused client with numpads.
        awful.key({ modkey, 'Control', 'Shift' }, '#' .. numpads[i], function()
                      if client.focus then
                          local tag = client.focus.screen.tags[i]
                          if tag then
                              client.focus:toggle_tag(tag)
...
globalkeys = gears.table.join(globalkeys,
    awful.key({ modkey }, '#' .. numpads[#numpads], function()
                  local screen = awful.screen.focused()
    ...
    awful.key({ modkey, 'Shift' }, '#' .. numpads[#numpads], function()
                  if client.focus then
